...
# Automated CI/CD Infrastructure with Terraform, Packer, and GitHub Actions
...
We can test the deployed infrastructure by accessing the user interfaces of all the deployed components. We can browse the various screens of Jenkins, Nexus, and SonarQube using loadbalancer URL before releasing infrastructure to the development team. Visible assessment of cloud infrastructure in the AWS portal is another manual way of testing the deployment.

Note: SonarQube is accessing PostgreSQL database for storing it's information and analytics report.

![AMIs](Images/Produced/AMI.JPG)

The figure above illustrates machine images generated by the GitHub Actions workflow for Jenkins, Nexus, SonarQube, and PostgreSQL, emphasizing immutable configuration.

![Instances](Images/Produced/Instances.JPG)

The above figure shows deployed EC2 instances from the above-mentioned AMIs (machine images).

![Loadbalancer](Images/Produced/lb.JPG)

![LB-targetgroups](Images/Produced/lb-targetgroup.JPG)

We can observe the load balancer and its associated target groups. Consequently, Jenkins, Nexus, and SonarQube are accessible via the Load Balancer on distinct ports. All EC2 instances, except for the Bastion Host, were deployed in private subnets. To access the private EC2 machines via SSH, initial login to the Bastion Host is required, followed by the necessary actions. Direct access to private EC2 machines is restricted to enhance security measures.
